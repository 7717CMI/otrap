...
export function filterData(
  ...
  console.log('🔍 filterData called with:', {
    ...
    effectiveLevel: effectiveAggregationLevel,
    viewMode: filters.viewMode,
    geographies: filters.geographies
  })
  console.log('🔍 Total records:', data.length)

  // Debug: Show records available for each segment type
  const segmentTypeCounts: Record<string, number> = {}
  data.forEach(r => {
    const st = r.segment_type || 'undefined'
    segmentTypeCounts[st] = (segmentTypeCounts[st] || 0) + 1
  })
  console.log('🔍 Records by segment type:', segmentTypeCounts)
  console.log('🔍 Filtering for segment type:', filters.segmentType, '- expected records:', segmentTypeCounts[filters.segmentType] || 0)
  if (effectiveAggregationLevel !== null && effectiveAggregationLevel !== undefined) {
  ...
  const filtered = data.filter((record) => {
    ...
    if (effectiveAggregationLevel !== undefined && effectiveAggregationLevel !== null) {
      ...
      // - Also include leaf records that don't have hierarchy info (they'll be summed by segment name)
      if (effectiveAggregationLevel === 2) {
        ...
        } else if (record.is_aggregated === false || recordLevel === null || recordLevel === undefined) {
          // Include leaf records without hierarchy info - they'll be summed by their segment name
          // This ensures we don't miss data when hierarchy info is incomplete
        } else {
          return false
        }
      } else {
    ...
    // DEBUG: Log first record's segment type check
    if (data.indexOf(record) === 0) {
      console.log('🔍 SEGMENT TYPE CHECK (first record):', {
  ...
  if (typeof window !== 'undefined') {
    ...
    if (sampleRecord) {
      ...
      // Show what segment types and segments are in the filtered results
      const filteredSegmentTypes = [...new Set(filtered.map(r => r.segment_type))]
      const filteredSegments = [...new Set(filtered.map(r => r.segment))]
      const filteredGeographies = [...new Set(filtered.map(r => r.geography))]

      console.log('🔍 FILTERED RESULTS:', {
        count: filtered.length,
        segmentTypes: filteredSegmentTypes,
        segments: filteredSegments,
        geographies: filteredGeographies,
        sampleValues: filtered.slice(0, 3).map(r => ({
          segment: r.segment,
          geography: r.geography,
          value2024: r.time_series[2024]
        }))
      })
...
export function prepareIntelligentMultiLevelData(
  ...
  const uniqueSegmentTypes = [...new Set(records.map(r => r.segment_type))]
  const uniqueSegments = [...new Set(records.map(r => r.segment))]

  console.log('📊 prepareIntelligentMultiLevelData received:', {
    recordCount: records.length,
    viewMode,
    segmentTypes: uniqueSegmentTypes,
  ...
  // Calculate total for 2024 to verify aggregation
  const total2024 = records.reduce((sum, r) => sum + (r.time_series[2024] || 0), 0)
  console.log('📊 Expected total for 2024 (sum of all records):', total2024)
  ...
  return years.map(year => {
    ...
    segmentGroups.forEach((groupRecords, key) => {
      // Strategy depends on view mode:
      // - In geography-mode: SUM all records for each geography (aggregate across segments)
      // - In segment-mode: Use the most appropriate record for each segment

      if (viewMode === 'geography-mode') {
        // Geography mode: SUM all records for this geography
        // This aggregates all segments within a geography into a single value
        const leafRecords = groupRecords.filter(r => !r.is_aggregated)

        if (leafRecords.length > 0) {
          // Sum all leaf records for this geography
          dataPoint[key] = leafRecords.reduce((sum, r) =>
            sum + (r.time_series[year] || 0), 0
          )
        } else {
          // No leaf records - sum aggregated records (but be careful of double-counting)
          // Use only the most granular aggregated records
          const minLevel = Math.min(...groupRecords.map(r => r.aggregation_level ?? 999))
          const recordsAtMinLevel = groupRecords.filter(r => (r.aggregation_level ?? 999) === minLevel)
          dataPoint[key] = recordsAtMinLevel.reduce((sum, r) =>
            sum + (r.time_series[year] || 0), 0
          )
        }
      } else {
        // Segment mode: Use the most appropriate record for this segment
