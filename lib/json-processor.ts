...
async function processSegmentTypeAsync(
  ...
  // DEBUG: Log segment type processing results
  const recordsByType: Record<string, number> = {}
  const sum2024ByType: Record<string, number> = {}
  records.forEach(r => {
    recordsByType[r.segment_type] = (recordsByType[r.segment_type] || 0) + 1
    sum2024ByType[r.segment_type] = (sum2024ByType[r.segment_type] || 0) + (r.time_series[2024] || 0)
  })
  console.log(`🔧 processSegmentTypeAsync RESULTS for "${segmentType}":`, {
    segmentType,
...
export async function processJsonDataAsync(
  ...
  try {
    ...
    // DEBUG: Summary of all valueRecords after processing all segment types
    const allRecordsByType: Record<string, number> = {}
    const allSum2024ByType: Record<string, number> = {}
    valueRecords.forEach(r => {
      allRecordsByType[r.segment_type] = (allRecordsByType[r.segment_type] || 0) + 1
      allSum2024ByType[r.segment_type] = (allSum2024ByType[r.segment_type] || 0) + (r.time_series[2024] || 0)
    })
    console.log('🔧 ALL VALUE RECORDS SUMMARY:', {
      totalRecords: valueRecords.length,
      recordsByType: allRecordsByType,
      sum2024ByType: allSum2024ByType,
      uniqueSegmentTypes: [...new Set(valueRecords.map(r => r.segment_type))]
    })
