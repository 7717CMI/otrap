  ...
  const chartData = useMemo(() => {
    ...
    if (isOpportunityMode) {
      ...
      if (activeFilters.segments && activeFilters.segments.length > 0) {
        ...
        // CASE 2: No segments selected - aggregate by Geography + Level 1 (parent segments)
        // Group records by their geography AND level_1 segment to preserve separate bubbles per geography
        ...
        filteredRecords.forEach(record => {
          ...
          if (level1 && geography) {
            // Use composite key of geography + level_1 to preserve geography separation
        ...
        geoLevel1Groups.forEach((childRecords, groupKey) => {
          ...
          // Create aggregated record for the geography + parent combination
          const aggregatedRecord = {
            ...childRecords[0],
            geography: geography, // Preserve geography
          ...
          <div>
            ...
            >
              ...
              {data?.dimensions?.segments ? Object.keys(data.dimensions.segments)
                ...
                // Also exclude Region and Country from Product Type options
                .filter(option => {
                  // Always exclude Region and Country from Product Type filter
                  if (option === 'Region' || option === 'Country') {
                    return false
                  }
