...
export function YearRangeSlider() {
  ...
  // Define the historical end year (2023) and forecast start year (2025) based on base year 2024
  const historicalEndYear = 2023
  const forecastStartYear = 2025
  const displayBaseYear = 2024

  const setPredefinedRange = (range: 'historical' | 'forecast' | 'all') => {
    switch (range) {
      case 'historical':
        updateFilters({ yearRange: [start_year, historicalEndYear] })
        break
      case 'forecast':
        updateFilters({ yearRange: [forecastStartYear, forecast_year] })
        break
      case 'all':
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="text-xs text-black text-center">
        Base Year: <span className="font-medium text-black">{displayBaseYear}</span>
      </div>
    </div>
  )
}
