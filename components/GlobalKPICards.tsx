...
export function GlobalKPICards() {
  ...
  const kpiData = useMemo(() => {
    console.log('🎯 GlobalKPICards useMemo RUNNING - filters changed:', {
    ...
    // DEBUG: Log segment types in the dataset
    const datasetSegmentTypes = [...new Set(dataset.map(r => r.segment_type))]
    const recordCountsBySegmentType: Record<string, number> = {}
    dataset.forEach(r => {
      recordCountsBySegmentType[r.segment_type] = (recordCountsBySegmentType[r.segment_type] || 0) + 1
    })
    console.log('🎯 GlobalKPICards DEBUG:', {
      filterSegmentType: filters.segmentType,
      targetSegmentType,
      availableSegmentTypes: segmentTypes,
      datasetSegmentTypes,
      recordCountsBySegmentType,
      totalRecords: dataset.length,
      sampleRecords: dataset.slice(0, 5).map(r => ({
        segment: r.segment,
    ...
    const filteredSegmentTypes = [...new Set(globalRecords.map(r => r.segment_type))]
    const sum2024 = globalRecords.reduce((acc, r) => acc + (r.time_series[2024] || 0), 0)
    console.log('🎯 GlobalKPICards FILTERED:', {
      targetSegmentType,
      filteredRecordCount: globalRecords.length,
      filteredSegmentTypes,
      sum2024,
      filteredSegments: [...new Set(globalRecords.map(r => r.segment))],
      filteredGeographies: [...new Set(globalRecords.map(r => r.geography))]
    })
